...
st.set_page_config(layout="wide", page_title="ThangLong Ultimate V37", page_icon="🐲")
...
# ==========================================
# 🧠 XỬ LÝ DỮ LIỆU (FULL TCBS - NO YAHOO)
# ==========================================
...
# 1. LẤY GIÁ & CHART TỪ TCBS
def get_data_tcbs(ticker):
    try:
...
# 2. LẤY CHỈ SỐ CƠ BẢN TỪ TCBS (THAY THẾ YAHOO)
def get_fundamental_tcbs(ticker):
    try:
        # API lấy thông tin cơ bản (P/E, P/B, ROE...)
...
def load_data_final(ticker, time):
    # 1. Load Chart
    df_calc = get_data_tcbs(ticker)
    df_chart = pd.DataFrame()
    
    if not df_calc.empty:
        # Cắt data cho chart
        if time == "1d": df_chart = df_calc.tail(100) 
        elif time == "5d": df_chart = df_calc.tail(5)
    ...
    # 2. Load Fundamental (New V37)
    fund_data = get_fundamental_tcbs(ticker)
...
# ==========================================
# 🧠 PHÂN TÍCH CƠ BẢN (REWRITTEN FOR TCBS V37)
# ==========================================
def analyze_fundamental(fund_data):
    score = 0; details = []
    
    # Mặc định an toàn nếu không có dữ liệu
    pe = 0; pb = 0; roe = 0; debt_equity = 0
    
    if not fund_data:
        return {"health": "KHÔNG DỮ LIỆU", "color": "gray", "details": ["Không lấy được thông tin doanh nghiệp"]}

    try:
        # Lấy dữ liệu từ TCBS JSON
        pe = fund_data.get('priceToEarning', 0)
        pb = fund_data.get('priceToBook', 0)
        roe = fund_data.get('roe', 0) * 100 # TCBS trả về số thập phân
        debt_equity = fund_data.get('debtOnEquity', 0) * 100
        market_cap = fund_data.get('marketCap', 0) / 1000000000 # Tỷ đồng
        ...
        details.append(f"Vốn hóa: {market_cap:,.0f} tỷ")

    except:
        return {"health": "LỖI DỮ LIỆU", "color": "gray", "details": ["Lỗi xử lý dữ liệu"]}
...
if mode == "📘 Hướng Dẫn & Quy Tắc":
    ...
    * Xuất hiện **Shooting Star** hoặc **Engulfing Giảm (▼)**.
    ...
    with c1: symbol = st.text_input("Nhập Mã CP (Ví dụ: HPG, OIL, BSR)", value="HPG").upper()
    with c2: 
    ...
    if symbol:
        ...
        if not df_chart.empty and not df_calc.empty:
            try:
                ...
                # Lấy tên từ TCBS (nếu có) hoặc dùng mã
                    ...
                    with col_fund:
                        ...
                        with st.expander("🔍 Chi tiết Cơ Bản", expanded=True):
                            for d in fund['details']: 
                                if "cao" in d and "Nợ" in d: st.warning(f"⚠️ {d}") # Cảnh báo nợ cao
                ...
                with t5:
                    ...
                    with c1:
                        ...
                        st.info(f"Tên: {fund_data.get('shortName', 'N/A')}")
                        st.info(f"Sàn: {fund_data.get('exchange', 'N/A')}")
                    with c2:
                        st.subheader("Chỉ Số Chính")
                        st.write(f"EPS: {fund_data.get('eps', 0):,.0f} đ")
                        st.write(f"P/E: {fund_data.get('priceToEarning', 0):.1f}")
                        st.write(f"P/B: {fund_data.get('priceToBook', 0):.1f}")

            except Exception as e:
                st.error(f"⚠️ Có lỗi khi xử lý dữ liệu mã {symbol}. Chi tiết: {e}")
        else:
            st.error(f"❌ Không tìm thấy dữ liệu cho mã '{symbol}'. Hãy kiểm tra lại mã hoặc thử lại sau.")

elif mode == "📊 Bảng Giá & Máy Quét":
    st.title("📊 Máy Quét Siêu Hạng V37")
...
st.markdown('<div class="footer">Developed by <b>Thăng Long</b> | V37 Ultimate - Full Data</div>', unsafe_allow_html=True)
